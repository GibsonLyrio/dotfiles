...
yay -S --noconfirm --needed btop neofetch kitty zsh zinit ttf-meslo-nerd fzf
...
# ---------------------------------------------------------------------------- #
# Setting zsh as default shell for kitty terminal
# ---------------------------------------------------------------------------- #
...
# Only echo if does not have any other shell setup (to avoid duplicate)
if ! grep -q "^shell " "$KITTY_CONFIG"; then
    echo "shell $(which zsh)" >> "$KITTY_CONFIG"
fi
